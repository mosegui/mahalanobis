...
class Mahalanobis1D:
    ...
    """
    def __init__(self, array, calib_entries):
...
def Mahalanobis(input_array, calib_rows, nan_subst_method='median'):
    ...
    if input_array.shape[1] == 1:
        parent = Mahalanobis1D
    elif input_array.shape[1] > 1:
        parent = MahalanobisND
    else:
        raise ShapeError('Empty input array')

    class MahalanobisBenchmark(parent):
        ...
        def __init__(self, array, calib_entries, nan_method='median'):
            """Calibration is performed right away at instantiation time, as well as the calculation of the Mahalanobis
            distances ob the observations in the input array.
            ...
            super().__init__(self.array, self.calib_entries)
